...
const VerifyWizard: React.FC<VerifyWizardProps> = ({ address, isCompressed }) => {
  ...
  const handleVerifyClick = async () => {
    ...
    if (isCompressed) {
      ...
      await verifyCreator(umi, { ...assetWithProof, creator: creatorSigner }).sendAndConfirm(umi)
    } else {
  ...
  return (
    <div>
      {wallet.connected ? (
        <button onClick={handleVerifyClick} className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Verify</button>
      ) : (
        <WalletMultiButton />
      )}
    </div>
